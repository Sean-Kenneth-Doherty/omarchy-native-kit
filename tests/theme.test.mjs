...
test('verifies a directory of committed dogfood app contracts', () => {
  ...
  assert.equal(report.ok, true);
  assert.equal(report.appCount, 69);
  assert.equal(report.verifiedCount, 69);
  ...
  assert.ok(report.reports.some((app) => app.appName === 'backup-key-recovery-planner'));
  ...
  assert.ok(report.reports.some((app) => app.appName === 'dns-resolver-policy-lab'));
  assert.ok(report.reports.some((app) => app.appName === 'emergency-access-runbook'));
  assert.ok(report.reports.some((app) => app.appName === 'environment-variable-auditor'));
  assert.ok(report.reports.some((app) => app.appName === 'firewall-rule-planner'));
  assert.ok(report.reports.some((app) => app.appName === 'flatpak-permission-auditor'));
  assert.ok(report.reports.some((app) => app.appName === 'font-rendering-tuner'));
  assert.ok(report.reports.some((app) => app.appName === 'focus-flight-recorder'));
  assert.ok(report.reports.some((app) => app.appName === 'gpg-keyring-policy-lab'));
  assert.ok(report.reports.some((app) => app.appName === 'hardware-token-manager'));
  assert.ok(report.reports.some((app) => app.appName === 'icon-theme-coordinator'));
  assert.ok(report.reports.some((app) => app.appName === 'incident-credential-rotator'));
  assert.ok(report.reports.some((app) => app.appName === 'input-method-studio'));
  assert.ok(report.reports.some((app) => app.appName === 'journal-timeline-inspector'));
  assert.ok(report.reports.some((app) => app.appName === 'locale-timezone-manager'));
  assert.ok(report.reports.some((app) => app.appName === 'mime-defaults-workbench'));
  ...
  assert.ok(report.reports.some((app) => app.appName === 'pinentry-prompt-router'));
  assert.ok(report.reports.some((app) => app.appName === 'power-profile-switchboard'));
  assert.ok(report.reports.some((app) => app.appName === 'proxy-environment-router'));
  assert.ok(report.reports.some((app) => app.appName === 'privacy-indicator-monitor'));
  ...
  assert.ok(report.reports.some((app) => app.appName === 'saml-assertion-trust-lab'));
  assert.ok(report.reports.some((app) => app.appName === 'webauthn-passkey-policy-center'));
  ...
  assert.ok(report.reports.some((app) => app.appName === 'vpn-profile-coordinator'));
...
test('catalogs committed dogfood apps', () => {
  ...
  assert.equal(catalog.appCount, 69);
  assert.equal(catalog.verifiedCount, 69);
  assert.deepEqual(
