...
test('emits CSS and JSON schemas', () => {
  ...
  const darktable = toDarktableCss(theme.tokens);
  ...
  assert.match(darktable, /Generated by Omarchy Native Kit for darktable/);
  assert.match(darktable, /@define-color omarchy_accent #7aa2f7;/);
  assert.match(darktable, /@define-color omarchy_photo_canvas #262626;/);
  assert.match(darktable, /#center, #preview, #thumbtable/);
...
test('verifies a directory of committed dogfood app contracts', () => {
  ...
  assert.equal(report.ok, true);
  assert.ok(report.appCount >= 69);
  assert.equal(report.verifiedCount, report.appCount);
...
test('catalogs committed dogfood apps', () => {
  ...
  assert.ok(catalog.appCount >= 69);
  assert.equal(catalog.verifiedCount, catalog.appCount);
  for (const appName of [
    ...
    assert.ok(catalog.apps.some((app) => app.name === appName), `missing ${appName}`);
  }
